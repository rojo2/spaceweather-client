...
class SolarWindGraphData extends React.Component {
  ...
  render() {
    ...
    const xTicks = xAxis.map((value, index, list) => {
      ...
      return (
        <g className="tick" transform={transform} key={value}>
          <text x="0" y="0" style={{ textAnchor: 'middle' }}>
    ...
    const yTemperatureTicks = utils
      .ticks(minTemperatureY, maxTemperatureY)
      .map((value, index, list) => {
        const x = 16
        const y = utils.interpolate(
          (value - minTemperatureY) / (maxTemperatureY - minTemperatureY),
          height,
          0
        )
        const transform = `translate(${x},${y})`
    ...
    const yDensityTicks = utils
      .ticks(minDensityY, maxDensityY)
      .map((value, index, list) => {
        const x = -16
        const y = utils.interpolate(
          (value - minDensityY) / (maxDensityY - minDensityY),
          height,
          0
        )
        const transform = `translate(${x},${y})`
    ...
    return (
      <g>
        {xLines}
        <g className="Graph__density">{density}</g>
        <g className="Graph__temperature">{temperature}</g>
    ...
    return (
      ...
      >
        <g data-width={dataWidth} transform={transform}>
          ...
          />
          <line
            className="Graph__timeline"
            y1="0"
            y2={height}
            x1={this.props.value * this.mWidth}
            x2={this.props.value * this.mWidth}
          />
        </g>
      </svg>
    )
